...
用户提供论文 PDF 路径后，读取 PDF 内容，然后按照"耿同学六式"逐一检测。
...
读取 PDF → 提取全文文本、表格数据、figure caption
...
报告结构：
1. 论文信息（标题、作者、期刊、DOI、发表年份）
2. 综合评定等级
3. 详细发现（位置、描述、证据、严重程度）
4. 辣评（犀利但幽默的总结点评）
5. 建议后续行动（联系作者、PubPeer、期刊举报、机构举报）
6. 免责声明
...
1. **图像分析局限**：读取 PDF 中的图片是以视觉方式理解的，无法进行像素级 ELA（Error Level Analysis）或 EXIF 元数据分析。对于需要像素级比对的情况，标注"建议使用专业工具进一步验证"。
